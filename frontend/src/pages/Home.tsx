...
export default function Home() {
  return (
    <>
      <Seo title="Kharbanda Saw Dust Store | Pioneer in Wood Industry Since 1974" description="Wood dust processing, manufacturing and bulk supply from Delhi and Yamuna Nagar. Explore sawdust, wood powder, shavings and wooden flakes trusted since 1974." schema={organizationSchema} />
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-5 py-20 sm:py-28 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:py-32">
          ...
          <div className="relative mx-auto w-full max-w-md lg:ml-auto" data-testid="hero-brand-image-panel">
            ...
            <div className="relative overflow-hidden bg-[#FBF9F5] p-3 shadow-2xl shadow-black/30">
              ...
              <div className="absolute bottom-7 left-7 right-7 flex items-center justify-between bg-[#183A2B]/95 px-4 py-3 text-white"><span className="text-xs font-bold uppercase tracking-[0.12em]" data-testid="hero-image-caption">Wood dust processing</span><span className="text-[#D7A16F]" data-testid="hero-image-year">ESTD. 1974</span></div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="border-b border-[#E0D7C6] bg-[#F4EFE6]" data-testid="trust-strip">
        <div className="mx-auto grid max-w-7xl grid-cols-2 gap-3 px-5 py-4 sm:grid-cols-3 lg:grid-cols-6 lg:px-8">
          {["Trusted Since 1974", "Second-Generation Business", "Nationwide Supply", "Multiple Wood Species", "Bulk Material Supply", "Application-Focused"].map((item, index) => <div key={item} className="flex min-h-24 flex-col items-center justify-center border border-[#E0D7C6] bg-[#FBF9F5] px-3 py-5 text-center text-xs font-bold uppercase leading-5 tracking-[0.12em] text-[#3D2314] shadow-sm sm:px-4" data-testid={`trust-item-${index}`}><span className="mb-2 font-heading text-lg text-[#C68B59]">0{index + 1}</span>{item}</div>)}
        </div>
      </section>
