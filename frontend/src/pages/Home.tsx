    ...
    <>
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-5 py-20 sm:py-28 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:py-32">
          ...
          <div className="relative mx-auto w-full max-w-md lg:ml-auto" data-testid="hero-brand-image-panel">
            ...
            <div className="relative overflow-hidden bg-[#FBF9F5] p-3 shadow-2xl shadow-black/30">
              <img src={productImages.providedBrand} alt="Kharbanda Saw Dust Store branded product sack showing wood dust processing and product information" className="aspect-[1.2/1] w-full object-cover object-top" data-testid="hero-brand-image" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="border-b border-[#E0D7C6] bg-[#F4EFE6]" data-testid="trust-strip">
        <div className="mx-auto grid max-w-7xl grid-cols-2 gap-3 px-5 py-4 sm:grid-cols-3 lg:grid-cols-6 lg:px-8">
          {["Trusted Since 1974", "Second-Generation Business", "Nationwide Supply", "Multiple Wood Species", "Industrial Material Supply", "Application-Focused"].map((item, index) => <div key={item} className="flex min-h-24 flex-col items-center justify-center border border-[#E0D7C6] bg-[#FBF9F5] px-3 py-5 text-center text-xs font-bold uppercase leading-5 tracking-[0.12em] text-[#3D2314] shadow-sm sm:px-4" data-testid={`trust-item-${index}`}><span className="mb-2 font-heading text-lg text-[#C68B59]">0{index + 1}</span>{item}</div>)}
        </div>
      </section>
      ...
      <section className="section-shell" data-testid="home-locations-section">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2"><div className="border border-[#E0D7C6] bg-white p-6" data-testid="home-office-location"><MapPin className="size-5 text-[#2D6A4F]" aria-hidden="true" /><h3 className="mt-5 font-heading text-xl font-bold" data-testid="home-office-heading">Delhi Office</h3><p className="mt-2 text-sm leading-6 text-[#57534E]" data-testid="home-office-address">{brand.office}</p><a href="https://www.google.com/maps/search/?api=1&query=Pronod%20Tech%2C%20Sawan%20Park%2C%20Delhi" target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-[#183A2B]" data-testid="home-office-directions">Get directions <ArrowRight className="size-4" aria-hidden="true" /></a></div><div className="border border-[#E0D7C6] bg-white p-6" data-testid="home-factory-location"><Truck className="size-5 text-[#2D6A4F]" aria-hidden="true" /><h3 className="mt-5 font-heading text-xl font-bold" data-testid="home-factory-heading">Yamuna Nagar Factory</h3><p className="mt-2 text-sm leading-6 text-[#57534E]" data-testid="home-factory-address">{brand.factory}</p><a href="https://www.google.com/maps/search/?api=1&query=Plot%20No.%2042%2C%20Industrial%20Estate%20Phase-II%2C%20Yamuna%20Nagar%20-%20135001%20%28Haryana%29" target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-[#183A2B]" data-testid="home-factory-directions">Get directions <ArrowRight className="size-4" aria-hidden="true" /></a></div></div>
      </section>
