...
import InteractiveBrandEmblem from "@/components/InteractiveBrandEmblem";
import { applications, brand, defaultWhatsappMessage, faqs, productImages, products, whatsappUrl } from "@/lib/siteData";
...
const organizationSchema = {
  ...
  "@graph": [
    ...
    { "@type": "LocalBusiness", name: `${brand.name} - Delhi Office`, telephone: brand.phones[0], address: { "@type": "PostalAddress", streetAddress: "K-11, Sawan Park, Ashok Vihar - Phase 3, Near Picnic Hut", addressLocality: "Delhi", postalCode: "110052", addressCountry: "IN" } },
...
export default function Home() {
  return (
    <>
      <Seo title="Kharbanda Saw Dust Store | Pioneer in Wood Industry Since 1974" description="Wood dust processing, manufacturing and industrial supply from Delhi and Yamuna Nagar. Explore sawdust, wood powder, shavings and wooden flakes trusted since 1974." schema={organizationSchema} />
      <section className="hero-section relative overflow-hidden" data-testid="hero-section">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-5 py-20 sm:py-28 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:py-32">
          <div className="max-w-2xl text-white">
            ...
            <p className="mt-7 max-w-xl text-lg leading-8 text-white/78 sm:text-xl" data-testid="hero-description">Wood dust processing, manufacturing &amp; industrial supply since 1974. Reliable material conversations for manufacturers, procurement teams and industrial buyers across India.</p>
            <div className="mt-10 flex flex-wrap gap-x-7 gap-y-3 text-xs font-semibold uppercase tracking-[0.12em] text-white/60" data-testid="hero-proof-points"><span data-testid="hero-proof-established">Established 1974</span><span data-testid="hero-proof-nationwide">Nationwide supply</span><span data-testid="hero-proof-species">Different wood species</span></div>
          </div>
          <div className="relative mx-auto w-full max-w-md lg:ml-auto" data-testid="hero-brand-image-panel"><InteractiveBrandEmblem /></div>
        </div>
      </section>

      <section className="border-b border-[#E0D7C6] bg-[#F4EFE6]" data-testid="trust-strip">
        <div className="mx-auto grid max-w-7xl grid-cols-2 gap-3 px-5 py-4 sm:grid-cols-3 lg:grid-cols-6 lg:px-8">
          {["Trusted Since 1974", "Second-Generation Business", "Nationwide Supply", "Multiple Wood Species", "Industrial Material Supply", "Application-Focused"].map((item, index) => <div key={item} className="depth-surface flex min-h-24 flex-col items-center justify-center border border-[#E0D7C6] bg-[#FBF9F5] px-3 py-5 text-center text-xs font-bold uppercase leading-5 tracking-[0.12em] text-[#3D2314] shadow-sm sm:px-4" data-testid={`trust-item-${index}`}><span className="mb-2 font-heading text-lg text-[#C68B59]">0{index + 1}</span>{item}</div>)}
        </div>
      </section>
      ...
      <section className="section-shell border-y border-[#E0D7C6] bg-[#F4EFE6]" data-testid="home-faq-section">
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-2">{faqs.slice(0, 4).map((faq) => { const slug = faq.question.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, ""); return <details key={faq.question} className="group border border-[#E0D7C6] bg-[#FBF9F5] p-5" data-testid={`faq-${slug}`}><summary className="cursor-pointer list-none pr-6 text-sm font-bold text-[#1C1917] marker:hidden" data-testid={`faq-question-${slug}`}>{faq.question}<span className="float-right text-[#2D6A4F] transition group-open:rotate-45" aria-hidden="true">+</span></summary><p className="pt-4 text-sm leading-6 text-[#57534E]" data-testid={`faq-answer-${slug}`}>{faq.answer}</p></details>; })}</div>
      </section>
      ...
      <section className="section-shell" data-testid="home-locations-section">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2"><div className="depth-surface border border-[#E0D7C6] bg-white p-6" data-testid="home-office-location"><MapPin className="size-5 text-[#2D6A4F]" aria-hidden="true" /><h3 className="mt-5 font-heading text-xl font-bold" data-testid="home-office-heading">Delhi Office</h3><p className="mt-2 text-sm leading-6 text-[#57534E]" data-testid="home-office-address">{brand.office}</p><a href="https://www.google.com/maps/search/?api=1&query=Kharbanda%20Saw%20Dust%20Store%2C%20Sawan%20Park%2C%20Ashok%20Vihar%2C%20Delhi" target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-[#183A2B]" data-testid="home-office-directions">Open pinned location <ArrowRight className="size-4" aria-hidden="true" /></a></div><div className="depth-surface border border-[#E0D7C6] bg-white p-6" data-testid="home-factory-location"><Truck className="size-5 text-[#2D6A4F]" aria-hidden="true" /><h3 className="mt-5 font-heading text-xl font-bold" data-testid="home-factory-heading">Yamuna Nagar Factory</h3><p className="mt-2 text-sm leading-6 text-[#57534E]" data-testid="home-factory-address">{brand.factory}</p><a href="https://www.google.com/maps/search/?api=1&query=Satnam%20Industries%2C%20Industrial%20Area%2C%20Yamuna%20Nagar%2C%20Haryana" target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-[#183A2B]" data-testid="home-factory-directions">Open pinned location <ArrowRight className="size-4" aria-hidden="true" /></a></div></div>
      </section>
